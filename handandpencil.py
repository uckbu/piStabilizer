...
def compute_euler_angles(hand_landmarks, w, h):
    ...
    # A fallback roll measure from the same axis
...
def compute_lavender_pen_roll(frame):
    ...
    # Approximate HSV range for a lavender/pastel purple.
    # You likely need to tweak these values to match your actual pen & lighting.
    ...
    # Assume the largest valid contour is the pen
...
def main():
    ...
    # Smoothing dictionaries
    smoothed = {"pitch": 0.0, "yaw": 0.0, "roll": 0.0}
    alpha = 0.3  # smoothing factor

    # Calibration: press 'c' to capture the neutral (upright) state
    ...
    try:
        while True:
            ...
            # 1) Hand-based orientation for pitch & yaw
            ...
            # 2) Lavender pen detection for roll
            pen_roll = compute_lavender_pen_roll(frame)
            
            # 3) Combine results
            # We trust the hand for pitch & yaw, and the pen's contour for roll if available
            # If pen detection fails, fallback to the hand-based roll
            ...
            # 4) Calibration: compute errors relative to reference
            ...
            # 6) Display servo angles in the top-left
            font_scale = 0.6
            color = (0, 255, 0)
            thickness = 2
            ...
            if key == ord('q'):
                ...
                # Calibrate current smoothed angles as neutral
                ref_angles = (smoothed["pitch"], smoothed["yaw"], smoothed["roll"])
                print(f"Calibrated: Pitch {ref_angles[0]:.2f}, "
                      f"Yaw {ref_angles[1]:.2f}, Roll {ref_angles[2]:.2f}")
    finally:
        cap.release()
